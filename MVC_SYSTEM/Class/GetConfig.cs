...
namespace MVC_SYSTEM.Class
{
    public class GetConfig
    {
        ...
        public string GetStatusEndcontract(string kod, int? NegaraID, int? SyarikatID)
        {
            var status = "";
            if (kod != null)
            {
                status = db.tblOptionConfigsWebs.Where(x => x.fldOptConfFlag1 == "statusendcontract" && x.fldOptConfValue == kod && x.fld_NegaraID == NegaraID && x.fld_SyarikatID == SyarikatID).Select(s => s.fldOptConfDesc).FirstOrDefault();
            }
            return status;
        }

        public string GetStatusOnLeave(string kod, int? NegaraID, int? SyarikatID)
        {
            var status = "";
            if (kod != null)
            {
                status = db.tblOptionConfigsWebs.Where(x => x.fldOptConfFlag1 == "statusonleave" && x.fldOptConfValue == kod && x.fld_NegaraID == NegaraID && x.fld_SyarikatID == SyarikatID).Select(s => s.fldOptConfDesc).FirstOrDefault();
            }
            return status;
        }


        public string GetStatusSickDeath(string kod, int? NegaraID, int? SyarikatID)
        {
            var status = "";
            if (kod != null)
            {
                status = db.tblOptionConfigsWebs.Where(x => x.fldOptConfFlag1 == "statusickdeath" && x.fldOptConfValue == kod && x.fld_NegaraID == NegaraID && x.fld_SyarikatID == SyarikatID).Select(s => s.fldOptConfDesc).FirstOrDefault();
            }
            return status;
        }
        ...
        public string GetStatusAbscondent(string kod, int? NegaraID, int? SyarikatID)
        {
            var status = "";
            if (kod != null)
            {
                status = db.tblOptionConfigsWebs.Where(x => x.fldOptConfFlag1 == "statusabscondent" && x.fldOptConfValue == kod && x.fld_NegaraID == NegaraID && x.fld_SyarikatID == SyarikatID).Select(s => s.fldOptConfDesc).FirstOrDefault();
            }
            return status;
        }
        ...
        public List<tblOptionConfigsWeb> GetConfigDesc(int? NegaraID, int? SyarikatID, string[] Flag)
        {
            List<tblOptionConfigsWeb> tblOptionConfigsWeb = new List<tblOptionConfigsWeb>();

            tblOptionConfigsWeb = db.tblOptionConfigsWebs.Where(x => Flag.Contains(x.fldOptConfFlag1) && x.fld_NegaraID == NegaraID && x.fld_SyarikatID == SyarikatID && x.fldDeleted == false).ToList();

            return tblOptionConfigsWeb;
        }

        public List<tbl_FlightDestination> GetFlightDestination(int? NegaraID, int? SyarikatID)
        {
            List<tbl_FlightDestination> tbl_FlightDestination = new List<tbl_FlightDestination>();

            tbl_FlightDestination = db.tbl_FlightDestination.Where(x => x.fld_NegaraID == NegaraID && x.fld_SyarikatID == SyarikatID && x.fld_Deleted == false).ToList();

            return tbl_FlightDestination;
        }
    }
}