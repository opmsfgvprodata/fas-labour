...
namespace MVC_SYSTEM.Class
{
    public class GetTriager
    {
        ...
        public List<Guid> StatusWorker(int PurposeLookUp)
        {
            string Host, Catalog, UserID, Pass = "";
            string Purpose = "LABOUR";
            Connection.GetConnection(out Host, out Catalog, out UserID, out Pass, 0, 1, 1, Purpose);
            var Labourdb = MVC_SYSTEM_Models.ConnectToSqlServer(Host, Catalog, UserID, Pass);
            List<Guid> WorkerID = new List<Guid>();

            switch (PurposeLookUp)
            {
                case 1:
                    WorkerID = Labourdb.tbl_LbrEndContract.Where(x => x.fld_Deleted == false).Select(s => s.fld_LbrRefID.Value).ToList();
                    break;
                case 2:
                    WorkerID = Labourdb.tbl_LbrAbsconded.Where(x => x.fld_Deleted == false).Select(s => s.fld_LbrRefID.Value).ToList();
                    break;
                case 3:
                    WorkerID = Labourdb.tbl_LbrOnLeave.Where(x => x.fld_Deleted == false).Select(s => s.fld_LbrRefID.Value).ToList();
                    break;
                case 4:
                    WorkerID = Labourdb.tbl_LbrSickDeath.Where(x => x.fld_Deleted == false).Select(s => s.fld_LbrRefID.Value).ToList();
                    break;
            }
